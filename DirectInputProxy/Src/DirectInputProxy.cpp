...
void SuspendAllThreadsButCurrent() {
    ...
    if (Thread32First(s_Snapshot, &s_ThreadEntry)) {
        do {
            if (s_ThreadEntry.dwSize >= FIELD_OFFSET(THREADENTRY32, th32OwnerProcessID) + sizeof(s_ThreadEntry.
                    th32OwnerProcessID) &&
...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpReserved) {
    ...
    if (fdwReason == DLL_PROCESS_ATTACH) {
        ...
        o_DirectInput8Create = reinterpret_cast<DirectInput8Create_t>(GetProcAddress(
            g_OriginalDirectInput, "DirectInput8Create"
        ));
        ...
        std::transform(
            s_ExecutableName.begin(), s_ExecutableName.end(), s_ExecutableName.begin(),
