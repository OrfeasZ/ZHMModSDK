...
public:
	...
	T Read()
	{
		...
		else if (this->data && this->data->size() > 0)
		{
			data = *reinterpret_cast<T*>(reinterpret_cast<char*>(this->data->data()) + position);
	...
	char* ReadChars(const size_t size)
	{
		...
		{
			memcpy(buffer, reinterpret_cast<char*>(this->data->data()) + position, size);
	...
	void ReadBytes(T* data, size_t count)
	{
		...
		{
			memcpy(data, this->data->data(), count);
