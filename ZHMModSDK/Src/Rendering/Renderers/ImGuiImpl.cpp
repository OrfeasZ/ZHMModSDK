...
void ImGui_ImplDX12_RenderDrawData(
    ImDrawData* draw_data, ID3D12GraphicsCommandList* ctx, ID3D12DescriptorHeap* desc_heap
) {
    ...
    if (fr->VertexBuffer == NULL || fr->VertexBufferSize < draw_data->TotalVtxCount) {
        ...
        if (g_pd3dDevice->CreateCommittedResource(
            &props, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_GENERIC_READ, NULL,
    ...
    if (fr->IndexBuffer == NULL || fr->IndexBufferSize < draw_data->TotalIdxCount) {
        ...
        desc.Flags = D3D12_RESOURCE_FLAG_NONE;
        if (g_pd3dDevice->CreateCommittedResource(
            &props, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_GENERIC_READ, NULL, IID_PPV_ARGS(&fr->IndexBuffer)
        ) < 0)
            return;
    }
...
bool ImGui_ImplDX12_CreateDeviceObjects() {
    ...
    // Create the root signature
    {
        ...
        g_pd3dDevice->CreateRootSignature(
            0, blob->GetBufferPointer(), blob->GetBufferSize(), IID_PPV_ARGS(&g_pRootSignature)
        );
        blob->Release();
    }
    ...
    // Create the vertex shader
    {
        ...
        if (FAILED(
            D3DCompile(vertexShader, strlen(vertexShader), NULL, NULL, NULL, "main", "vs_5_0", 0, 0, &vertexShaderBlob,
                NULL)
        ))
            return false;
    ...
    // Create the pixel shader
    {
        ...
        if (FAILED(
            D3DCompile(pixelShader, strlen(pixelShader), NULL, NULL, NULL, "main", "ps_5_0", 0, 0, &pixelShaderBlob,
                NULL)
        )) {
    ...
    HRESULT result_pipeline_state = g_pd3dDevice->CreateGraphicsPipelineState(
