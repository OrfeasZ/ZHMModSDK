...
public:
    static DetourTrampoline* CreateTrampoline(
        uintptr_t p_TargetAddress, void* p_AdditionalInstructions = nullptr, size_t p_AdditionalInstructionsSize = 0
    ) {
        if (g_Trampolines == nullptr) {
            ...
            auto s_AllocAddress = ALIGN_TO(
                reinterpret_cast<uintptr_t>(GetModuleHandleA(nullptr)) + ModSDK::GetInstance()->GetImageSize(),
            ...
            while (s_QueryResult != 0 && (s_MemInfo.State != MEM_FREE || reinterpret_cast<uintptr_t>(s_MemInfo.
                BaseAddress) + s_MemInfo.RegionSize < s_AllocEnd)) {
                s_AllocAddress = ALIGN_TO(
                    reinterpret_cast<uintptr_t>(s_MemInfo.BaseAddress) + s_MemInfo.RegionSize,
                    s_SysInfo.dwAllocationGranularity
                );
            ...
            if (s_QueryResult == 0) {
                ...
                Logger::Warn(
                    "Could not find a free memory region for trampoline storage. Allocating anywhere and praying."
                );
                s_AllocAddress = 0;
            }

            Logger::Trace(
                "Attempting to allocate trampoline storage at {}.", fmt::ptr(reinterpret_cast<void*>(s_AllocAddress))
            );

            g_Trampolines = static_cast<DetourTrampoline*>(VirtualAlloc(
                reinterpret_cast<void*>(s_AllocAddress), sizeof(DetourTrampoline) * MAX_TRAMPOLINES,
        ...
        if (g_TrampolineCount >= MAX_TRAMPOLINES) {
            Logger::Error(
                "Could not create trampoline because we have reached the max number of trampolines allowed ({}). You can raise this limit by modifying MAX_TRAMPOLINES in HookImpl.h.",
                MAX_TRAMPOLINES
            );
            return 0;
        }
...
public:
    PatternHook(
        const char* p_HookName, const char* p_Pattern, const char* p_Mask,
...
public:
    PatternCallHook(
        ...
        HookImpl<ReturnType(Args...)>(
            p_HookName, InstallDetourAndGetOriginal(p_HookName, p_Pattern, p_Mask, p_Detour)
        ) {}
...
private:
    typename Hook<ReturnType(Args...)>::OriginalFunc_t InstallDetourAndGetOriginal(
        ...
        if (s_Distance >= INT32_MAX || s_Distance <= INT32_MIN) {
            Logger::Trace(
                "Detour for hook '{}' is too far from the original call ({} - {} = {}). Creating trampoline.",
                p_HookName, fmt::ptr(p_Detour), fmt::ptr(reinterpret_cast<void*>(m_Target + 5)), s_Distance
            );
            ...
            if (s_Distance >= INT32_MAX || s_Distance <= INT32_MIN) {
                Logger::Error(
                    ...
                    fmt::ptr(reinterpret_cast<void*>(s_TrampolineAddress)),
...
private:
    void* GetTarget(const char* p_HookName, const char* p_ModuleName, const char* p_FunctionName) const {
        ...
        if (s_Module == nullptr) {
            Logger::Error(
                "Could not load requested module '{}' for hook '{}' (error: {}).", p_ModuleName, p_HookName,
                GetLastError()
            );
            return nullptr;
        }
        ...
        if (s_Function == nullptr) {
            Logger::Error(
                "Could not find requested function '{}' in module '{}' for hook '{}' (error: {}).", p_FunctionName,
                p_ModuleName, p_HookName, GetLastError()
            );
            return nullptr;
        }
